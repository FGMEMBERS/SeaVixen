...
fuelUpdate = func {
    ...
    AllEngines = props.globals.getNode("engines").getChildren("engine");
		...
		port_rate_gph =PortEngine.getNode("fuel-flow-gph",1);
		stbd_rate_gph =StbdEngine.getNode("fuel-flow-gph",1);
...
#		print ( " stbd_fuel ",stbd_fuel.getValue() );
		port_fuel_consumed = port_fuel.getValue() or 0;
		stbd_fuel_consumed = stbd_fuel.getValue() or 0;
		total = port_fuel_consumed + stbd_fuel_consumed;
		port_fuel.setDoubleValue(0);
		stbd_fuel.setDoubleValue(0);

		used_lbs += total;
		rate_lbspm = (port_rate_gph.getValue() + stbd_rate_gph.getValue()) * 6.8/60;
	...
	setprop("/consumables/fuel/total-fuel-used-lbs", used_lbs);
	setprop("/consumables/fuel/total-fuel-flow-lbspm", rate_lbspm);
